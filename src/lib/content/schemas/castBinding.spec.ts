...
import { castBindingSchema, identityMaskSchema } from './castBinding.js';
...
describe('castBindingSchema', () => {
	...
	it('parses an identityMask (issue #31)', () => {
		const result = castBindingSchema.safeParse({
			characterRef: LUCY_ID,
			roleInStory: 'quest-giver',
		...
		expect(result.success).toBe(true);
		expect(result.success && result.data.identityMask).toEqual({
			maskedDisplayName: 'Unbekannt',
			revealCondition: 'flag:lucy-identified'
		});
	});

	it('leaves identityMask undefined when omitted', () => {
		const result = castBindingSchema.parse({ characterRef: LUCY_ID, roleInStory: 'bystander' });
		expect(result.identityMask).toBeUndefined();
	});
});

describe('identityMaskSchema', () => {
	it('rejects an empty maskedDisplayName', () => {
		const result = identityMaskSchema.safeParse({
			maskedDisplayName: '',
			revealCondition: 'flag:lucy-identified'
		});
		expect(result.success).toBe(false);
	...
	it('rejects a revealCondition that is not a valid symbolic ref', () => {
		const result = identityMaskSchema.safeParse({
			maskedDisplayName: 'Unbekannt',
			revealCondition: 'not a valid ref!'
		});
		expect(result.success).toBe(false);
	});
});
