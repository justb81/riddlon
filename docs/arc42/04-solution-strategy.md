...
| **Privacy / no cloud**    | Inference resolves _player-configured endpoint → native browser model → WebLLM on WebGPU_, in that order. The endpoint is opt-in and empty by default, so an untouched install is fully local; no cloud SDK is a dependency at all, the endpoint path is plain `fetch`. | [§8.4](./08-crosscutting-concepts.md#84-local-llm-inference)                                                         |
