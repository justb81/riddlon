...
| **Privacy / no cloud**    | Inference resolves _native browser model → WebLLM on WebGPU → Gemini BYOK_, in that order. The build aliases every cloud SDK to a throwing stub so a cloud dependency cannot creep in unnoticed.                                                | [§8.4](./08-crosscutting-concepts.md#84-local-llm-inference)                                                         |
...
## Structural decisions that follow from this
