...
| TC4 | **No cloud SDKs.** `package.json` carries no Firebase, OpenAI, Gemini or Transformers.js dependency, and the one deliberate cloud path (Gemini BYOK) is plain `fetch`. | Nothing in the build enforces this today — see [§11.2](./11-risks-and-technical-debt.md#112-technical-risks). Adding such a dependency is a review question, not a failing check. |
