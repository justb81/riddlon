...
| TC4 | **No cloud SDKs.** `package.json` carries no Firebase, OpenAI, Gemini or Transformers.js dependency; the player-configured OpenAI-compatible endpoint is plain `fetch`. This is a bundle-weight rule, not a policy one — reaching an external model is allowed, shipping an SDK for it is not. | Nothing in the build enforces this — see [§11.2](./11-risks-and-technical-debt.md#112-technical-risks). Adding such a dependency is a review question, not a failing check. |
