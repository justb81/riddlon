...
| --- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Offline capability**            | After installation the app must be fully playable without a network connection, AI inference included — whenever a local backend is used. A player who configures their own inference endpoint trades this away knowingly (for a server on their own network, only for LAN rather than nothing). No feature may require a server round trip. |
| 2   | **Privacy / no cloud dependency** | Local in-browser inference is preferred wherever the device can manage it. Nothing about a playthrough leaves the device unless the player explicitly configures their own inference endpoint — often a server on their own machine, in which case nothing leaves it at all.                                                                 |
