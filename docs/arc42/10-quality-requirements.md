...
| Q2  | The player opens the app on a device with no WebGPU and no built-in model, and no Gemini key.                   | The app boots, the library and settings work, and every thread states plainly why no message can be generated. Nothing crashes.   |
